...
describe("encodeSyncPayload / decodeSyncPayload", () => {
  it("round-trips an empty payload", () => {
    ...
    expect(encoded.startsWith("BR5.")).toBe(true);
  ...
  it("still decodes legacy BR1. payloads", () => {
    ...
    const encoded = encodeSyncPayload(data).replace(/^BR5\./, "BR1.");
    const decoded = decodeSyncPayload(encoded);
    expect(decoded.subscriptions[0].name).toBe("Legacy");
  });

  it("still decodes legacy BR2. payloads", () => {
    const data: BurnRateData = {
      subscriptions: [makeSubscription({ name: "From v3", costCents: 2000 })],
      trials: [],
      theme: "dark",
    };
    const encoded = encodeSyncPayload(data).replace(/^BR5\./, "BR2.");
    const decoded = decodeSyncPayload(encoded);
    expect(decoded.subscriptions[0].name).toBe("From v3");
  });

  it("still decodes legacy BR3. and BR4. payloads", () => {
    const data: BurnRateData = {
      subscriptions: [makeSubscription({ name: "From v4", costCents: 3000 })],
      trials: [],
      theme: "dark",
    };
    const br3 = encodeSyncPayload(data).replace(/^BR5\./, "BR3.");
    expect(decodeSyncPayload(br3).subscriptions[0].name).toBe("From v4");
    const br4 = encodeSyncPayload(data).replace(/^BR5\./, "BR4.");
    expect(decodeSyncPayload(br4).subscriptions[0].name).toBe("From v4");
  });
